...
NsgR=MigRule1
VM=Migrator
OS=MicrosoftWindowsServer:WindowsServer:2019-Datacenter-with-Containers:17763.1339.2007101755
...
az network traffic-manager profile create --name Mig-TM -g $RG --routing-method Priority --unique-dns-name $G
az network traffic-manager endpoint create --name OnPrem --profile-name Mig-TM -g $RG --type externalEndpoints --endpoint-status Enabled --priority 1 --target $onprem
az network traffic-manager endpoint create --name Migrated --profile-name Mig-TM -g $RG --type externalEndpoints --endpoint-status Enabled --priority 2 --target $migrated
